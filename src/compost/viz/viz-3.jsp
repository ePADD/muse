...
force.link.add(pv.Line)
  .fillStyle(function(d, p) (colors(a_nodes[p.target].group)))
...
var selected_n = -1;
force.link.add(pv.Line)
    .fillStyle(function(d, p) (colors(all_nodes[p.target].group)))
    .strokeStyle(function() this.fillStyle().darker())
...
force.node.add(pv.Dot)
    ...
    //.fillStyle(function(d) (((!d.isGroup)?(group_matrix[this.index][selected_g]):(selected_g == d.group)) && !(d.isGroup)) ? "red" : (((selected_n == -1) || (d.isGroup) || ((!d.isGroup) && (share_common_group[this.index][selected_n]==1))) ? colors(d.group) : "white"))
    //.fillStyle(function(d) ( ((this.index < addressBook.length)?(group_matrix[this.index][selected_g]):(selected_g == d.group)) && !(d.group==undefined)) ? "red" : colors(d.group))
    .strokeStyle(function() this.fillStyle().darker())
    //.title(function(d) d.nodeName)
    ...
    .event("mousedown", pv.Behavior.drag())
    .event("drag", force)
    .event("point", function(d) {selected_g = d.group; selected_n = this.index; this.active(this.index).parent;})
...
var instr = new Array();
instr.push("Network layout with links colored according to the group");
...
<p>
Go to Visualization: <a href="viz-2.jsp">Previous</a> | <a href="viz-4.jsp">Next</a>
<br/>
