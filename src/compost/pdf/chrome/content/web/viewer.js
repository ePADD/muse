...
var RenderingQueue = (function RenderingQueueClosure() {
    ...
    RenderingQueue.prototype = {
        enqueueDraw: function RenderingQueueEnqueueDraw(item) {
            ...
            return;
            // as no redraw required, no need for queueing.
...
var PDFView = {
    ...
    getDestinationHash: function pdfViewGetDestinationHash(dest) {
        ...
        if (dest instanceof Array) {
            var destRef = dest[0];
            // see navigateTo method for dest format
            var pageNumber = destRef instanceof Object ?
    ...
    function setupAnnotations(content, scale) {
        ...
        function createCommentAnnotation(type, item) {
            ...
            if (!item.content) {
                ...
                function annotationImageOver() {
                    this.nextSibling.removeAttribute('hidden');
                },
                false);

                image.addEventListener('mouseout',
                function annotationImageOut() {
    ...
    this.endLayout = function textLayerBuilderEndLayout() {
        ...
        function renderTextLayer() {
            ...
            if (textDiv.dataset.textLength > 0) {
                ...
                if (textDiv.dataset.textLength > 1) {
                    // avoid div by zero
...
function keydown(evt) {
    ...
    while (curElement) {
        ...
        return;
        // ignoring if the 'controls' element is focused
...
//this is our code - probably need to refactor it into a new js file 
//TODO 
...
function addTextNodes(node, arr, insert_delimiters) {
    ...
    return null;
    // ignore google bar because it causes spurious hits w/the name of the user.
    ...
    if (textNodesParsedWithoutBreak > 10)
    {
        ...
        setTimeout(function() {
            addTextNodes(node, arr, insert_delimiters);
        },
        1);
        return;
    }
    ...
    return;
    // we don't want to highlight our own nodes!
    if (typeof node.tagName !== 'undefined')
    if (node.tagName.toUpperCase() == 'SCRIPT' || node.tagName.toUpperCase() == 'NOSCRIPT')
    return;
    // don't look inside scripts, and even noscripts
        ...
        if (insert_delimiters && typeof node.tagName !== 'undefined')
        {
            if (arr && arr.length > 0 && arr[arr.length - 1] != null)
            {
                ...
                if ('!?.'.indexOf(lastText[lastText.length - 1]) < 0)
                {
                    ...
                    if (tag == 'H1' || tag == 'H2' || tag == 'H3' || tag == 'H4' || tag == 'H5' || tag == 'H6' || tag == 'DIV' || tag == 'P' || tag == 'LI' || tag == 'TD')
                    {
...
function render_rounder()
...

 {
    ...
    jQuery("#roundrr_container").roundrrwheel(
    {
        ...
        centerY: 60,
        animationEffect: 1,
        //(1:off, 5:light,10:heavy, 15:subtle)
        selectEvent: "click",
...
function driver(pageNumber)
 {
    var textNodesOnPage = addTextNodes($("#pageContainer" + pageNumber)[0], null, true);
    ...
    for (var i = 0; i < textNodesOnPage.length; i++)
    {
        ...
        {
            // var words= textNodesOnPage[i].data.split(" ");
            //for(var t=0;t<words.length;t++)
            textOnPage = textOnPage.rtrim();
            if (textOnPage.charAt(textOnPage.length - 1) == "-")
            textOnPage = textOnPage.substring(0, textOnPage.length - 1) + textNodesOnPage[i].data;
            else
            textOnPage += ' ' + textNodesOnPage[i].data;
        }
        //textOnPage += ' ' + punycode.toASCII(textNodesOnPage[i].data);
    }

    textOnPage = textOnPage.substring(0, 2900);
    // alert(textOnPage);
    /* textOnPage = textOnPage.replace(/\,/g,' ');
		  textOnPage = textOnPage.replace(/\[/g,' ');
		  textOnPage = textOnPage.replace(/\]/g,' ');
    ...
    // alert(textOnPage);
    var test_highlight = Array();
    test_highlight[0] = 'INRIA';
    //jQuery(document.body).highlight(test_highlight,'yellow');
    ...
    xhr.onreadystatechange = function(data) {
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                ...
                test_highlight = Array();
                for (var t = 0; t < result.length && t < 5; t++)
                test_highlight[t] = result[t].text;

                try {
                    $("#pageContainer" + pageNumber).highlight(test_highlight, 'yellow', pageNumber);

                }
                catch(err)
                {
                    }
                for (var t = 0; t < result.length && t < 10; t++)
                {
                    var imagelink = result[t].imageURL;
                    var cnt = pageNumber * 10 + t;


                    $('.personal' + cnt).qtip({
                        // Simply use an HTML img tag within the HTML string
                        content: '<img src="' + imagelink + '" alt="Personal~" width="100" height="100" />'
                    });
                }

                if (pageNumber == 1)
                {
                    var htm = "<div id='container' class='container'><div id='roundrr_navigation'> <a href='#' id='next'></a><a href='#' id='prev'></a></div>";

                    htm += "<div class='roundrr_center'></div>";

                    htm += "<div class='roundrr_speech'></div>";

                    htm += "<div id='roundrr_caption'></div>";

                    htm += "<div id='roundrr_pick'></div>";

                    htm += "<div id='roundrr_container'><ul class='list'>";

                    for (var t = 0; t < result.length && t < 10; t++)
                    {
                        var imagelink = result[t].imageURL;
                        htm += "<li class='item'><div class='my_class'>";
    ...
    var data = 'refText=' + encoded_page;
    var url = 'http://localhost:8080/Surface/getProfile.jsp?refText=' + encoded_page;
...
function my_selection_callback()
 {
    ...
    //if (!selected.obj.id)
    selected.obj.id = "test1";

    $('body').append('<a id="test2" href="javascript:void(0);" title="test1" class="jhighlight" rel=' + selected.text + '>insert</a>');
    $('body').append('<a href="javascript:void(0);" id="close">close</a>');
    ...
    xhr.onreadystatechange = function(data) {
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                ...
                try {
                    $("#pageContainer1").highlight(test_highlight, 'yellow', pageNumber);
...
$(document).ready(function() {
    ...
    $(document).bind("mouseup", my_selection_callback);
...
function doIt()
 {
    // alert("ok");
    var hits = eval('(' + muse_response.responseText + ')');
    ...
    var textNodes = textNodesOnPage;
    // as an optimization, we'll reuse the nodes we found earlier. addTextNodes($_('body')[0], null, false);

    for (var x = 0; x < textNodes.length; x++)
    if (textNodes[x] != null)
    {
        try {
            decorateTextNode(textNodes[x], true, 0);
        }
        catch(err) {
            alert(err);
        }
