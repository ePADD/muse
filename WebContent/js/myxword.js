...
//		inject_html ('<div id="muse-status" style="' + muse_status_style + '">' + content + '</div>');
      ...
      // ignore content that is not displayed in the page
		try {
			if (node.nodeType != 3) // don't do vis check for nodeType 3 (text nodes), as it fails unnecessarily on cs.stanford.edu
                if (false && ignore_invisible && !$_(node).is(':visible')) // flag debug
                {
			...
			if (insert_delimiters && typeof node.tagName !== 'undefined')
			{
				if (typeof arr != 'undefined' && arr && arr.length > 0 && arr[arr.length-1] != null)
				{
					...
					if ('!?.'.indexOf(lastText[lastText.length-1]) < 0)
					{
						...
						if (tag == 'H1' || tag == 'H2' || tag == 'H3' || tag == 'H4' || tag == 'H5' || tag == 'H6' || tag == 'DIV' || tag == 'P' || tag == 'LI' || tag == 'TD') // perhaps we should just check all tags that are not span.
						{
	...
	var muse_url = MUSE_URL + "/crossword";
	...
	$_('#muse-status').html('Generating a puzzle...');
	$_('#muse-status').show();

	var tn_url = '';
	
	var entities = ""; // provide entities if we have them, e.g. from wikipedia
	if ($_('#mw-content-text').length > 0)
	{
	    // see if we can find a flag
		var tn_urls = $_('.infobox img');
		if (tn_urls.length > 0)
		{
			tn_url = $_(tn_urls[0]).attr('src');
			if (tn_url.indexOf('http') != 0)
				tn_url = 'http:' + tn_url;
			if (tn_url.indexOf('Flag_of') < 0) // only take the tn url if it has the magic word flag_of
				tn_url = null;
		}
		...
		$_('.seealso').html('');
		$_('.mainarticle').html('');
		$_('.gallery').html(''); // see an example on HK Page
		$_('.wikitable').html('');
				
		textNodesOnPage = addTextNodes($_('#mw-content-text')[0], null, true, ignore_invisible);
		// nuke noisy meta-info
		as = $_('p a[href^="/wiki"]');	
		LOG ('all /wiki = ' + as.length);
		as = as.filter(function(idx) { 
			var len = $_(this).text().length; 
			if (len < 3 || len > 15) { return false; } 
			return true; 
		});
		as = as.map(function(idx) { return $_(this).text().replace(/ /, "_")});
		LOG ('after length filter = ' + as.length);
		LOG ('all /wiki = ' + as.length);
		entities = $.makeArray(as).join(" ");
		entities = entities.replace(/\u0101/g, 'a'); // canonicalize 'a's
	}
	else
...
//		LOG ('encoded page = ' + encoded_page);
    ...
    /*
    $('<form action="' + muse_url + '" method="POST">' + 
    	    '<input type="hidden" name="text" value="' + textOnPage + '">' +
    	    '</form>').submit();
    return;
    */
    var post_data = {text: textOnPage, refURL: document.URL, entities: entities, clearTabooWords: 'true', title: document.title.substring(0, document.title.indexOf(' -'))};
    if (tn_url != null)
    	post_data.tn = tn_url;
	...
	$_.ajax({
			url: muse_url, 
			type: 'POST',
//			data: {text: textOnPage, refURL: document.URL, entities: entities, clearTabooWords: 'true', symmetric: 'true', title: document.title.substring(0, document.title.indexOf(' -'))}, 
			data: post_data,
			dataType: 'text',
			...
			success: function(response) {
			    ...
			    window.location.href = MUSE_URL + '/crossword?useSessionXword=true';
			},
			error: function(response) {			    
		        ...
		        $_("#callout").html ('Sorry, the server is off. Please try later.');
...
function decorateTextNode(node, hits, hit_pats, anchors_entered) {
    ...
    for (var hit = 0; hit < hits.results.length; hit++)
    {
        ...
        if (hits.results[hit].indexScore > 0)
        {
            ...
            decorator.setAttribute('title',  'Associated with: ' + people_str + ' (Score: ' + parseInt(hits.results[hit].score*100)/100.0 + ' Page score: ' + parseInt(hits.results[hit].pageScore*100)/100.0 + ' Index score: ' + parseInt(hits.results[hit].indexScore*100)/100.0 + ' for ' + hits.results[hit].text + ')'); // set it otherwise "undefined" shows up in bottom left corner
...
var open_popup = function(hit_details) 
{
    ...
    popup += '<p><a href=\'' + hit_details.url + '\' target="_">View ' + /* hit_details.nMessages + *//* nMessages is currently inaccurate */ ' message' + (hit_details.nMessages > 1 ? 's':'') + '</a><hr/>';
...
var muse_show_filter = function() {
	...
	$_('.muse-overlay').height($(document).height());		
    $_('.muse-overlay').width($(document).width());		
    $_('.muse-overlay').show();		
...
function create_callout(hits) {
	function html_for_terms(hits)
	{
		...
		for (var i = 0; i < hits.results.length; i++) {
			...
			n_hits++;
			var title = 'Score on page: ' + hits.results[i].pageScore + '; in archive: ' + hits.results[i].nMessages; 
...
function decoratePage(muse_response) {
	...
	if (!hits || typeof hits.results == 'undefined') {
		...
		LOG('muse returned an error: ' + hits.displayError);
		return; // do nothing
	}
