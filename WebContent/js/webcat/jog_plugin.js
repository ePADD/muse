...
// LOG ('jog settings = ' + dump_obj(settings));
      ...
      // inject jog_div
        var jog_div = '<div id="jog_div" style="position:fixed;display:none;top:0px;left:0px;opacity:0.9;z-index:1000000000">' 
    			+ '<table><tr><td>	<img id="jog_img" src="' + settings.reel_prefix + '0.png' + '" width="' + settings.width + '"></img>	</td>';
...
function Jog(settings, height, width, trackWidth, forward_handler, backward_handler)
{
	...
	if (!settings.disabled) {
		...
		$(document).click(x); // same click handler on jog itself needed also
...
Jog.prototype.showJog = function (mouseX, mouseY, x, y)  // mouseX/Y is the actual mouse pos, for computing current compartment
{
	if (settings.disabled) {
	...
	var top = y-(this.height/2)-30; // -10 so we are slightly inside
	var left = x-(this.width/2);
	LOG ('showing jog at ' + x + ',' + y + ' left,top=' + left + ',' + top + ' h=' + this.height + ' w= ' + this.width);
...
/* could try CSS3 on supported browsers:
	$('#jog_img').css('-moz-transform', 'rotate(' + 45*c + 'deg)');
	$('#jog_img').css('-webkit-transform', 'rotate(' + 45*c + 'deg)');
	$('#jog_img').css('-transform', 'rotate(' + 45*c + 'deg)');
*/
...
Jog.prototype.createMouseMoveHandler = function () {
    ...
    var currentX = event.clientX, currentY = event.clientY; // clientX/Y are offset w.r.t. client window. http://www.gtalbot.org/DHTMLSection/PositioningEventOffsetXY.html
...
Jog.prototype.createClickHandler = function() {
	...
	LOG ('at click ' + dump_obj(evt));
        var currentX = evt.pageX, currentY = evt.pageY;
...
//	LOG ('frame contents: ' + frameContents);
	...
	img = settings.reel_prefix + ((currentCompartment+7)%8) + '.png';
	$('#jog_img').attr('src', img);
//	LOG ('jog img is ' + img);
