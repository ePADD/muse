  ...
  $.fn.jog = function(options) {
	...
	// debug func: print all members directly in o (and not its supertypes)
	  ...
	  var settings = {
		      ...
		      reel1        : 'images/reel21.png',
		      reel2        : 'images/reel22.png',
		      reel_prefix  : 'images/movieReel',
		      width		   : 250,
...
LOG ('jog settings = ' + dump_obj(settings));
      ...
      // inject jog_div
    	$('body').append('<div id="jog_div" style="position:absolute;display:none;top:0px;left:0px;opacity:0.9;z-index:1001">'
    			+ '<table><tr><td>	<img id="jog_img" src="' + settings.reel1 + '" width="' + settings.width + '"></img>	</td>'
    			+ '<td valign="middle">&nbsp;&nbsp 	<span id="jog_text" style="' + settings.text_css + '">&nbsp;</span><br/>	</td></tr></table>	</div>');
...
Jog.prototype.showJog = function (x, y, forward_handler, backward_handler)
{
	if (settings.disabled) {
	...
	var top = y-(this.height/2)-0;
	var left = x-(this.width/2);
	LOG ('showing jog at ' + x + ',' + y + ' top,left=' + top + ',' + left + ' width = ' + this.width);
	$('#jog_div').css('top', top); // -50 to make the mouse start at the ready-to-go point on the jog
...
Jog.prototype.createMouseMoveHandler = function () {
	...
	function moveJogDynamic()
	{
	    ...
	    if (compartment == myJog.prevCompartment)
	    	return;
	   // LOG('new compartment: ' + compartment);
	    ...
	    //	LOG ('moving forward: compartment ' + myJog.prevCompartment + ' -> ' + compartment);
	    	myJog.forward_handler();
	    } else if ((compartment == ((myJog.prevCompartment+7) % 8)) || (compartment == ((myJog.prevCompartment+6) % 8))) {
	    //	LOG ('moving backward: compartment ' + myJog.prevCompartment + ' -> ' + compartment);
...
Jog.prototype.createClickHandler = function() {
	...
	return function (evt)
	{
		...
		{
			LOG ('not registering click because the target is an a or span ' + target.tagName + ' ' + target.className);
...
//		LOG ('handleclick called');
		if (myJog.enabled) 
		{
			...
			LOG ('Jog summoned at ' + evt.pageX, evt.pageY);
	...
	var pages_jq = $(settings.page_selector);
...
//	LOG ('frame contents: ' + frameContents);
	...
	var img = ((currentPage/N_PAGES_IN_FRAME) % 2 == 0) ? settings.reel1 : settings.reel2;
	// override!
	
	//img = 'images/movieReel' + (currentPage%8) + '.png';
	img = settings.reel_prefix + (currentPage%8) + '.png';
