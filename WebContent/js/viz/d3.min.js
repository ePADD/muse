(function(){var o=null;d3={version:"0.26.1"};if(!Date.now)Date.now=function(){return+new Date};if(!Object.create)Object.create=function(a){function b(){}b.prototype=a;return new b};function x(a){return Array.prototype.slice.call(a)}function y(a){return typeof a=="function"?a:function(){return a}}d3.ascending=function(a,b){return a<b?-1:a>b?1:0};d3.descending=function(a,b){return b<a?-1:b>a?1:0};d3.merge=function(a){return Array.prototype.concat.apply([],a)};
...
d3.ak_text=function(b,e){var f=new XMLHttpRequest;if(arguments.length==3)f.overrideMimeType(b);else e=b;f.onreadystatechange=function(){if(f.readyState==4)e(f.status<300&&f.responseText?f.responseText:o)};f.send(o)};d3.ak_json=function(b){return d3.text("application/json",function(e){b(e&&JSON.parse(e))})};
...
i=0,j=g.length;i<j;i++){var m=g[i];if(m)return c.call(m,m.__data__,i)}return o}a.select=function(c){return b(function(d){return d.querySelector(c)})};a.selectAll=function(c){return e(function(d){return x(d.querySelectorAll(c))})};a.filter=function(c){for(var d=[],h,g,i,j=0,m=a.length;j<m;j++){g=a[j];d.push(h=[]);h.parentNode=g.parentNode;h.parentData=g.parentData;for(var p=0,r=g.length;p<r;p++)if((i=g[p])&&c.call(i,i.__data__,p))h.push(i)}return O(d)};a.data=function(c,d){function h(k,n){function q(Ea){return k.parentNode.appendChild(Ea)}
var l=0,t=k.length,u=n.length,s=Math.min(t,u),v=Math.max(t,u),z=[],A=[],w=[],B,C;if(d){s={};v=[];var D;C=n.length;for(l=0;l<t;l++){D=d.nodeKey(B=k[l]);if(D in s)w[C++]=k[l];else{s[D]=B;v.push(D)}}for(l=0;l<u;l++){if(B=s[D=d.dataKey(C=n[l])]){B.__data__=C;z[l]=B;A[l]=w[l]=o}else{A[l]={appendChild:q,__data__:C};z[l]=w[l]=o}delete s[D]}for(l=0;l<t;l++)if(v[l]in s)w[l]=k[l]}else{for(;l<s;l++){B=k[l];C=n[l];if(B){B.__data__=C;z[l]=B;A[l]=w[l]=o}else{A[l]={appendChild:q,__data__:C};z[l]=w[l]=o}}for(;l<
u;l++){A[l]={appendChild:q,__data__:n[l]};z[l]=w[l]=o}for(;l<v;l++){w[l]=k[l];A[l]=z[l]=o}}A.parentNode=z.parentNode=w.parentNode=k.parentNode;A.parentData=z.parentData=w.parentData=k.parentData;m.push(A);p.push(z);r.push(w)}var g=-1,i=a.length,j,m=[],p=[],r=[];if(typeof d=="string")d=va(d);if(typeof c=="function")for(;++g<i;)h(j=a[g],c.call(j,j.parentData,g));else for(;++g<i;)h(j=a[g],c);g=O(p);g.enter=function(k){return O(m).append(k)};g.exit=function(){return O(r)};return g};a.each=function(c){for(var d=
...
d3.svg.chord=function(){function a(g,i){var j=b(this,e,g,i),m=b(this,f,g,i);return"M"+j.c+("A"+j.r+","+j.r+" 0 0,1 "+j.p)+(j.k==m.k&&j.m==m.m?"Q 0,0 "+j.c:"Q 0,0 "+m.c+("A"+m.r+","+m.r+" 0 0,1 "+m.p)+("Q 0,0 "+j.c))+"Z"}function b(g,i,j,m){var p=i.call(g,j,m);i=c.call(g,p,m);j=d.call(g,p,m)+V;g=h.call(g,p,m)+V;return{r:i,k:j,m:g,c:[i*Math.cos(j),i*Math.sin(j)],p:[i*Math.cos(g),i*Math.sin(g)]}}var e=Ua,f=Va,c=Wa,d=Ka,h=La;a.radius=function(g){if(!arguments.length)return c;c=y(g);return a};a.source=
function(g){if(!arguments.length)return e;e=y(g);return a};a.target=function(g){if(!arguments.length)return f;f=y(g);return a};a.startAngle=function(g){if(!arguments.length)return d;d=y(g);return a};a.endAngle=function(g){if(!arguments.length)return h;h=y(g);return a};return a};function Ua(a){return a.source}function Va(a){return a.target}function Wa(a){return a.radius}
